...
void ProcessorGraph::StartProcessing(std::string run_group_id,
    ...
    if (state_ == GraphState::READY) {
        ...
        // runinfo_.reset( new RunInfo( terminate_signal_, context_,
        // run_identifier, destination, source ) );
        ...
        if (it->second.IsMap()) {
            ...
            for (YAML::iterator it2 = it->second.begin();
                ...
                try {
                    ...
                    // it2->second = processor->internal_UpdateState(
    ...
    for (YAML::iterator it = node.begin(); it != node.end(); ++it) {
        ...
        if (it->second.IsMap()) {
            ...
            for (YAML::iterator it2 = it->second.begin();
                ...
                try {
                    auto state_name = it2->first.as<std::string>();
                    // it2->second = processor->internal_RetrieveState(
                    // state_name );
                    auto pstate = processor->shared_state(state_name);
