...
void EventDelayed::Configure(const GlobalContext &context) {
    ...
    LOG_IF(WARNING, start_after_detection_() != start_after_stimulation_())
        << name()
        << "Event trigger lockout time:\n Be careful if you switch on ontime "
           "mode "
        << "where stimulation and detection are in the same time, "
        << "your options set will be interpreted as do a stimulation lock-out "
           "time. \n"
        << "Turn off both detection and stimulation if you want to "
           "deactivate. ";
}

void EventDelayed::CreatePorts() {
    data_in_port_ = create_input_port<EventType>(
        EventType::Capabilities(), PortInPolicy(SlotRange(1), false, 1));
    ...
    while (!context.terminated()) {
        ...
        if (!disabled_->get()) {
            ...
            if (delayed_event_->get()) {
                ...
                if (not(start_after_detection_() or
