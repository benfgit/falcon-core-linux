...
bool s_nonblocking_recv(zmq::socket_t &socket, std::string &s_message) {
    zmq::message_t message;
    zmq_msg_recv(reinterpret_cast<zmq_msg_t *>(&message), socket, ZMQ_NOBLOCK);
    if (!message.empty()) {
...
bool s_send(zmq::socket_t &socket, const std::string &string) {
    ...
    bool rc = zmq_msg_send(reinterpret_cast<zmq_msg_t *>(&message), socket, 0);
    return (rc);
}
...
bool s_sendmore(zmq::socket_t &socket, const std::string &string) {
    zmq::message_t message(string.size());
    memcpy(message.data(), string.data(), string.size());
    bool rc = zmq_msg_send(reinterpret_cast<zmq_msg_t *>(&message), socket, ZMQ_SNDMORE);
    return (rc);
}
